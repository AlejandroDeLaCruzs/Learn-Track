...
public class ValidadorInicioSesion {
    public static boolean esValido(String email, char[] contrasenia, VentanaPrincipal ventanaContenedor, ArrayList<Usuario> usuarios) {
        for (Usuario usuario : usuarios) {
            if (email.equals(usuario.getNombre())) {
                ...
                if (contraseniaIngresada.equals(usuario.getContrasenia())) {
                    ...
                    if (usuario instanceof Administrador) {
                        usuarioactivo = new Administrador(usuario.getIdusuario(), usuario.getNombre(), usuario.getContrasenia());
                    } else if (usuario instanceof Alumno) {
                        usuarioactivo = new Alumno(usuario.getIdusuario(), usuario.getNombre(), usuario.getContrasenia());
                    } else {
                        usuarioactivo = new Profesor(usuario.getIdusuario(), usuario.getNombre(), usuario.getContrasenia());
                    }
